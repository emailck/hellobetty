...
- Homework templates own reusable content and public assets; published homework rows are assignment instances that own immutable content snapshots, recipients, schedules, lifecycle, occurrences, submissions, and reviews.
- Inline publication creates a template and its first assignment instance atomically. Publishing by `templateId` snapshots the authorized template without copying prior recipients or schedule. Teachers may browse and reuse templates they created or that are referenced by an assignment instance currently visible through classroom scope, but may delete only their own templates; administrators manage the full library.
- Deleting a template sets linked instance `template_id` values to null and never removes instance snapshots or history. Legacy homework backfill is recorded as an idempotent data migration so deleted templates are not recreated on restart.
- Staff homework-instance detail uses the same database-derived scope as history and returns original recipients plus every ordered snapshot question for read-only image and sample-audio preview.
- The staff latest-cycle endpoint selects the greatest occurrence sequence whose schedule has started, returns every assigned student's occurrence, and derives `CHECKED_IN` only from occurrence `COMPLETED`; any nonzero partial submission is `IN_PROGRESS`, otherwise `NOT_STARTED`.
...
- Staff review applies to the latest submission for every picture-book or generic item, including server-scored objective answers. Review entry groups submitted occurrences by published homework instance, then opens occurrence-level conversations with submitted/reviewed/total counts; filters include accessible student, student name, homework, latest submission time, and pending/all review mode. Pending mode includes only instances with at least one latest unreviewed submission while each group retains complete matching-instance summary counts. New reviews accept only `SSS`, `SS`, `S`, `A`, or `B` while legacy `C`/`D` grades remain read-only, and automatic correctness remains independent from the human grade. Feedback audio must be uploaded privately with admin upload `purpose=FEEDBACK`, recorded in `feedback_uploads`, and stored under `/uploads/feedback/*`; public asset URLs, unregistered feedback URLs, another staff member's unconsumed upload, and URLs already consumed by another submission are not valid review feedback. Re-review without a new `feedbackAudioUrl` preserves existing feedback. A later attempt becomes the current unreviewed submission without rewriting prior attempts.
