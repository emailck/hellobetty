...
- Student homework history reads the server-owned paginated history newest-first and shows lifecycle, completion progress, and staff-review progress without reopening paused or archived work.
- The student homework list renders one row per due occurrence, including repeated occurrences with identical content, shows its `scheduledAt` dispatch date at the lower right, and shows a compact colored icon and label for `未查看`, `未完成`, `已完成`, or `老师已批改`; these states come from server-owned view sessions, completion counts, and latest-submission review counts.
...
- Administrator unscoped mobile publishing must load the full paginated authorized-student list before recipient selection.
