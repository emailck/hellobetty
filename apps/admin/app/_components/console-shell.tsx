...
import { Activity, BookOpen, ClipboardCheck, Coins, LayoutDashboard, LogOut } from "lucide-react";
import { usePathname, useRouter } from "next/navigation";

const NAV_ITEMS = [
  { href: "/dashboard", label: "运营账号", icon: LayoutDashboard },
  { href: "/homeworks", label: "作业管理", icon: BookOpen },
  { href: "/reviews", label: "作业批改", icon: ClipboardCheck },
  ...
  return <div className="console">
    <aside className="sidebar">
      ...
      <nav className="sidebar-nav" aria-label="管理工作区">
        {NAV_ITEMS.map(({ href, label, icon: Icon }) => <a key={href} className={`nav-link ${pathname === href || pathname.startsWith(`${href}/`) ? "active" : ""}`} href={href}><Icon size={17} />{label}</a>)}
      </nav>
